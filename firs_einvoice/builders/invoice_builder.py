...
class InvoiceBuilder:
    """Fluent builder for creating Invoice objects with Pydantic validation."""
    ...
    def __init__(self, business_context: Optional[BusinessContext] = None):
        ...
        Args:
            business_context: Optional business context for default supplier info
        """
        self._invoice_data = {}
    ...
    def with_invoice_date(self, invoice_date: Union[datetime, date, str]) -> 'InvoiceBuilder':
        """Set invoice date."""
        if isinstance(invoice_date, str):
            invoice_date = datetime.fromisoformat(invoice_date)
        elif isinstance(invoice_date, date) and not isinstance(invoice_date, datetime):
            invoice_date = datetime.combine(invoice_date, datetime.min.time())
        self._invoice_data["invoice_date"] = invoice_date
        return self
    
    def with_due_date(self, due_date: Union[datetime, date, str]) -> 'InvoiceBuilder':
        """Set due date."""
    ...
    def with_invoice_type(self, invoice_type: Union[InvoiceType, str]) -> 'InvoiceBuilder':
        """Set invoice type."""
    ...
    def with_original_invoice_number(self, original_invoice_number: str) -> 'InvoiceBuilder':
    ...
    def with_currency(
        ...
        if exchange_rate:
            ...
            raise ValueError(f"Exchange rate required for non-NGN currency: {currency}")
        
        return self
    
    def with_payment_details(self, payment_details: PaymentDetails) -> 'InvoiceBuilder':
        """Set payment details."""
    ...
    def validate(self) -> bool:
        ...
        if invoice_type in [InvoiceType.CREDIT, InvoiceType.DEBIT]:
            if not self._invoice_data.get("original_invoice_number"):
                raise ValueError(f"Original invoice number required for {invoice_type.value} notes")
