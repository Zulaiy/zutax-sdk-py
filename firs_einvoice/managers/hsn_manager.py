"""HSN (Harmonized System of Nomenclature) code management."""
...
class HSNManager:
    ...
    def initialize(cls) -> None:
        ...
        # Medical supplies
        cls._add_hsn_code('3004', 'Medicaments', 'MEDICAL', 0, True, VAT_EXEMPTION_REASONS['MEDICAL'])
        cls._add_hsn_code('3005', 'Wadding, gauze, bandages', 'MEDICAL', 0, True, VAT_EXEMPTION_REASONS['MEDICAL'])
        cls._add_hsn_code('3006', 'Pharmaceutical preparations', 'MEDICAL', 0, True, VAT_EXEMPTION_REASONS['MEDICAL'])
        
        # Basic food items
        cls._add_hsn_code('1001', 'Wheat and meslin', 'FOOD_BASIC', 0, True, VAT_EXEMPTION_REASONS['FOOD'])
        cls._add_hsn_code('1006', 'Rice', 'FOOD_BASIC', 0, True, VAT_EXEMPTION_REASONS['FOOD'])
        cls._add_hsn_code('0401', 'Milk and cream', 'FOOD_BASIC', 0, True, VAT_EXEMPTION_REASONS['FOOD'])
        
        # Infant products
        cls._add_hsn_code('1901', 'Infant food preparations', 'INFANT', 0, True, VAT_EXEMPTION_REASONS['INFANT'])
        cls._add_hsn_code('3401', 'Baby soap and cleansing preparations', 'INFANT', 0, True, VAT_EXEMPTION_REASONS['INFANT'])
        
        # Educational materials
        cls._add_hsn_code('4901', 'Books, brochures, leaflets', 'EDUCATION', 0, True, VAT_EXEMPTION_REASONS['EDUCATION'])
        cls._add_hsn_code('4902', 'Newspapers, journals', 'EDUCATION', 0, True, VAT_EXEMPTION_REASONS['EDUCATION'])
        
        # Standard taxable items
        cls._add_hsn_code('8471', 'Computers and computer peripherals', 'ELECTRONICS', 7.5, False)
        cls._add_hsn_code('8517', 'Telephones and telecommunication equipment', 'ELECTRONICS', 7.5, False)
        cls._add_hsn_code('8703', 'Motor cars and vehicles', 'AUTOMOTIVE', 7.5, False)
        cls._add_hsn_code('9403', 'Furniture', 'FURNITURE', 7.5, False)
        cls._add_hsn_code('6109', 'T-shirts, singlets', 'TEXTILES', 7.5, False)
    
    @classmethod
    def _add_hsn_code(cls, code: str, description: str, category: str, 
                     tax_rate: float, is_exempt: bool, exemption_reason: str = None) -> None:
    ...
    def get_category_hsn_codes(cls, category: str) -> List[HSNCode]:
        ...
        cls.initialize()
        return [hsn for hsn in cls._hsn_database.values() if hsn.category == category]
    ...
    def search_hsn(cls, search_term: str) -> List[HSNCode]:
        ...
        for hsn in cls._hsn_database.values():
            if (term in hsn.code.lower() or 
                term in hsn.description.lower() or 
                term in hsn.category.lower()):
                results.append(hsn)
