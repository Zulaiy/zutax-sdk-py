"""Tax-related Pydantic models (Zutax)."""
...
class Tax(FIRSBaseModel):
    ...
    rate: Decimal = Field(
        ..., ge=0, le=100, decimal_places=2, description="Tax rate percentage"
    )
    amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Tax amount"
    )
    exemption_reason: Optional[str] = Field(
        None, max_length=200, description="Tax exemption reason if applicable"
    )
...
class TaxDetail(FIRSBaseModel):
    ...
    taxable_amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Amount subject to tax"
    )
    tax_amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Calculated tax amount"
    )
    exempt_amount: Optional[Decimal] = Field(
        None, ge=0, decimal_places=2, description="Tax exempt amount"
    )
    description: Optional[str] = Field(
        None, max_length=200, description="Tax description or notes"
    )
...
class TaxBreakdown(FIRSBaseModel):
    ...
    subtotal: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Subtotal before tax"
    )
    ...
    taxable_amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Total taxable amount"
    )
    tax_details: List[TaxDetail] = Field(
        default_factory=list, description="Breakdown by tax category"
    )
    ...
    def tax_summary(self) -> Dict[str, Decimal]:
        ...
        for detail in self.tax_details:
            category = (
                detail.category
                if isinstance(detail.category, str)
                else detail.category.value
            )
...
class TaxCalculation(FIRSBaseModel):
    ...
    base_amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Base amount before tax"
    )
    ...
    tax_rate: Decimal = Field(
        ..., ge=0, le=100, decimal_places=2, description="Applied tax rate"
    )
    tax_amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Calculated tax amount"
    )
    total_amount: Decimal = Field(
        ..., ge=0, decimal_places=2, description="Total amount including tax"
    )

    # Tax breakdown by category
    vat_amount: Optional[Decimal] = Field(
        None, ge=0, decimal_places=2, description="VAT amount"
    )
    excise_amount: Optional[Decimal] = Field(
        None, ge=0, decimal_places=2, description="Excise duty amount"
    )
    customs_amount: Optional[Decimal] = Field(
        None, ge=0, decimal_places=2, description="Customs duty amount"
    )
    withholding_amount: Optional[Decimal] = Field(
        None, ge=0, decimal_places=2, description="Withholding tax amount"
    )

    # Exemption details
    is_exempt: bool = Field(default=False, description="Tax exemption status")
    exemption_reason: Optional[str] = Field(
        None, max_length=200, description="Tax exemption reason"
    )
    exemption_certificate: Optional[str] = Field(
        None, max_length=50, description="Tax exemption certificate number"
    )
