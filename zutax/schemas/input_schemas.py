"""Input validation schemas with relaxed validation for user input (Zutax)."""
...
class LineItemInput(BaseModel):
    ...
    unit_of_measure: Optional[Union[UnitOfMeasure, str]] = Field(
        default="UNIT"
    )
    unit_price: Union[Decimal, float, int, str] = Field(..., ge=0)
    discount_percent: Optional[Union[Decimal, float, int, str]] = Field(
        default=0
    )
    discount_amount: Optional[Union[Decimal, float, int, str]] = Field(
        default=None
    )
...
class BulkInvoiceInput(BaseModel):
    ...
    validate_only: bool = Field(
        default=False, description="Only validate without submission"
    )
