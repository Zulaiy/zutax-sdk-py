"""Invoice processing and validation utilities (Zutax)."""
...
from ..schemas.validators_impl import InvoiceValidator
...
class InvoiceProcessor:
    ...
    async def process_invoice(
        ...
        try:
            ...
            # 2. BUILD INVOICE
            builder = cls._create_invoice_builder(
                validation["data"], business_context
            )
    ...
    def _create_invoice_builder(
        ...
        builder.set_document_currency_code(
            input_data.get("document_currency_code", "NGN")
        )
        builder.set_tax_currency_code(
            input_data.get("tax_currency_code", "NGN")
        )
        ...
        builder.set_accounting_supplier_party(
            input_data["accounting_supplier_party"]
        )
        builder.set_accounting_customer_party(
            input_data["accounting_customer_party"]
        )

        # Line items
        for line_item_input in input_data["invoice_line"]:
            line_total_default = (
                line_item_input["invoiced_quantity"]
                * line_item_input["price"]["price_amount"]
            )
            line_item = {
                ...
                "unit_price": Decimal(
                    str(line_item_input["price"]["price_amount"])
                ),
                "unit_of_measure": line_item_input["price"].get(
                    "price_unit", "PIECE"
                ),
                "discount": Decimal(
                    str(line_item_input.get("discount_amount", 0))
                ),
                ...
                "line_total": Decimal(
                    str(
                        line_item_input.get(
                            "line_extension_amount", line_total_default
                        )
                    )
                ),
                "net_amount": Decimal(
